...
def _print_agent(a: dict):
    prompt = a.get("system_prompt", "")
    prompt_preview = (prompt[:80] + "...") if prompt and len(prompt) > 80 else prompt
    tools = a.get("tools") or []
    tools_str = ", ".join(tools) if tools else "无"
    typer.echo(f"  [{a['id']}] {a['name']}")
    typer.echo(f"      角色: {a['role']}")
    typer.echo(f"      工具: {tools_str}")
    if prompt_preview:
        typer.echo(f"      提示词: {prompt_preview}")
    typer.echo()
...
def list_agents():
    ...
    async def _run():
        ...
        try:
            ...
            if not agents:
                typer.echo("没有 Agent。使用 crewcraft agent add 添加。")
                return
            for a in agents:
                _print_agent(a)
        finally:
...
def add_agent(
    ...
    async def _run():
        ...
        try:
            ...
            agent = await client.create_agent(data)
    ...
    async def _run():
        ...
        try:
            ...
            for t in tasks:
                ...
                typer.echo(f"  [{t['id']}] {icon} {t['input'][:60]}")
                if t.get("result"):
                    typer.echo(f"      → {t['result'][:80]}")
                typer.echo()
        finally:
            await client.close()
    asyncio.run(_run())
